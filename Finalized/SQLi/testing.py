...
def search_and_predict():
	pauseAPScheduler()
	df = pd.read_json(r'sqlinjections.json', lines=True)
	...
	if len(df) >= 2:
		...
		# df_reduced['payload'] = df_reduced['payload'].apply(lambda x: str(x))
		...
		# print(df.head())
		# x_test = x_test[x_test['Sentence'].notnull()].copy()
		# x_test['Sentence'] = x_test['Sentence'].apply(lambda x: str(x))

		# vectorized = np.vstack(x_test['Sentence'].apply(lambda x: vectorize_payload(x)))
		# features_frequency = np.vstack([calc_frequency(x) for x in vectorized])
		...
		x_test['payload'] = x_test['payload'].apply(lambda x: str(x))
		print(x_test.head())
		number_of_observations = len(x_test)

		# Vectorizing and calculating frequencies for payloads
		vectorized = np.vstack(x_test['payload'].apply(lambda x: vectorize_payload(x)))
		features_frequency = np.vstack([calc_frequency(x) for x in vectorized])
