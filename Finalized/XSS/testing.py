...
def search_and_predict():
	pauseAPScheduler()
	# df = pd.read_csv(r'XSS_dataset.csv', sep=',', error_bad_lines=False)
	...
	if len(df) >= 2:
		...
		# df_reduced = df[df['Sentence'].notnull()].copy()
		...
		positive_threshold = 5*payloads_sd_sum
		negative_threshold = -5*payloads_sd_sum
